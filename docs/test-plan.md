...
| W22b ◈ | Branded export — logo in PDF | As a partner with an **uploaded PNG/JPEG logo** + `branded-invoicing` (create one via the studio), export any PDF | PDF header shows the **actual partner logo** top-left on a white header (title in ink, accent rule below); partners without a raster logo keep the name-text asphalt band; MSTS exports unchanged | ☐ |
| W23 | Branded export — CSV | As Alpine → **Export CSV** | CSV includes an Alpine-branded comment/header line | ☐ |
| W24 | MSTS default export unchanged | As the default MSTS login (no partner param) → export PDF/CSV | Exports still say **"MSTS Tolls One"**; PDF shows the same thin accent bar (Shell yellow) — this is a new, plan-mandated element and not a regression | ☐ |
...
| G0b ◈ | MSTS Tolls One — Classic replica | Studio → **Launch replica** (or open `/login?replica=classic`) → sign in | The COMPLETE MSTS Tolls One app in the classic blue theme: blue sidebar/actions/links, white surfaces — but identical name ("MSTS Tolls One"), both portals + switcher, all modules, SourceTags, full entity list, MSTS-branded exports. Sign-out returns to the replica's login (`?replica=classic`); removing the param restores Signage colors | ☐ |
...
| G20 | Generic portal purity — Alpine | Logged in as `tolls.alpine.com`, read the full sidebar + dashboard | Sidebar shows generic groups/labels **Overview→Dashboard, Fleet→Vehicles/Devices/Carriers, Tolling→Toll Products/Toll Coverage, Billing→Transactions/Reports/Invoices, Administration→Users & Access/Customer Onboarding, General→Support/Account**; page text contains **no** "MyTolls", "MyMST", "Toll2.0"/"Toll 2.0", or "MSTS Tolls One" — verified live via full body-text scan (all four strings absent) | — verified |
| G21 | Generic portal purity — Nordkap | Logged in as `tolls.nordkap.com`, read sidebar + dashboard | Same generic labels (Basic package hides Billing group + Administration entirely — confirmed absent); no MyTolls/MyMST/Toll2.0/MSTS Tolls One text — verified live | — verified |
